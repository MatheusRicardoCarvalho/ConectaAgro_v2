...
import { updateUser } from "../components/api/requests/update/updateUser";
...
export async function getThread(threadId: string) {
  try {
    ...
    throw new Error(error + " - erro ao obter a thread")
  }
}
...
export async function executeRun(
  ...
  let instructions = userInfo ? `Aqui estão algumas informações básicas desse usuário: ${userInfo}` : null;
  ...
  if (
    ...
    const toolOutputs = run.required_action.submit_tool_outputs.tool_calls
      .map((tool) => {
        ...
        } else if (tool.function.name === "update_user_data") {
          console.log("\nESTAMOS TRATANDO O UPDATE USER\n")
          const params = JSON.parse(tool.function.arguments);
          
          const data: RequestAgricultorFilterDTO = {
            nome: params.nome,
            idade: params.idade,
            ...
            tamanhoPropriedade: params.tamanhoPropriedade+'',
            culturas: params.culturas,
            questionarioBasico: true
          }

          updateUser(data, params.userId);
          resetUserThread('', params.userId)
          return {
            tool_call_id: tool.id,
            output: "Os Dados do usuário atualizados com sucesso.",
          };
        }
